...
func IndexPolicy(w http.ResponseWriter, r *http.Request) {
    ...
    switch app {
    case "flexi":
        columns = "policy_number, nomor_polis, kode_produk, kantor_cabang, no_rekening, premi, yearmonth, mulai_asuransi, selesai_asuransi, tanggal_lahir, url_sertifikat, risk, premi_refund, status, remark_refund"
    case "kpi", "afi":
    ...
    for rows.Next() {
        ...
        switch app {
        case "flexi":
            var premiRefund, remarkRefund sql.NullString
            if err := rows.Scan(&policy.PolicyNumber, &policy.NomorPolis, &policy.KodeProduk, &policy.KantorCabang, &policy.NoRekening, &policy.Premi, &policy.YearMonth, &policy.MulaiAsuransi, &policy.SelesaiAsuransi, &policy.TanggalLahir, &policy.URLSertifikat, &policy.Risk, &premiRefund, &policy.Status, &remarkRefund); err != nil {
                http.Error(w, err.Error(), http.StatusInternalServerError)
                return
            }
            if premiRefund.Valid {
                policy.PremiRefund = premiRefund.String
            } else {
                policy.PremiRefund = "" 
            }
            if remarkRefund.Valid {
                policy.RemarkRefund = remarkRefund.String
            } else {
                policy.RemarkRefund = "" 
            }
        case "kpi", "afi":
    ...
    // Ambil parameter "length" untuk menentukan jumlah data yang ditampilkan per halaman
    pageLength := 10
    if lengthStr := dashboardInput["length"]; lengthStr != "" {
        pageLength, _ = strconv.Atoi(lengthStr)
    }

    // Bagi data ke dalam halaman-halaman
    var paginatedPolicies []models.PolicyData
    page := 1
    startInd := (page - 1) * pageLength
    endInd := startInd + pageLength
    if len(policies) > startInd {
        if len(policies) < endInd {
            endInd = len(policies)
        }
        paginatedPolicies = policies[startInd:endInd]
    }
    ...
    responseData := map[string]interface{}{
        ...
        "total":           len(policies),
        "lastPage":        int(math.Ceil(float64(len(policies)) / float64(pageLength))),
        "status":          true,
