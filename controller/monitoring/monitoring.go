...
func IndexMon(w http.ResponseWriter, r *http.Request) {
	// Decode JSON request body
	var monitorInput map[string]string
	decoder := json.NewDecoder(r.Body)
	if err := decoder.Decode(&monitorInput); err != nil {
	...
	app := monitorInput["app"]
	start := monitorInput["start"]
	end := monitorInput["end"]
	email := monitorInput["email"]

	// Retrieve the length parameter
	length := 10
	if lenStr := monitorInput["length"]; lenStr != "" {
