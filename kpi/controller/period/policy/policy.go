...
func PolicyPeriodKpi(w http.ResponseWriter, r *http.Request) {

	// Koneksi ke database
	models.ConnectDatabase()
	db := models.DB
	// Query untuk mendapatkan semua data periode
	query := fmt.Sprintf("SELECT yearmonth, label FROM dashboard.sp_filter('admin', 'production|period', 'kpi')")
